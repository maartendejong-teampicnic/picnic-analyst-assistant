...
2. Read `~/picnic-analyst-assistant/TASKS.md` to identify the target task(s).

3. Resolve the `task-id` for each task (see Task ID resolution in ORCHESTRATOR.md).

4. For each task, check for an existing `tasks-output/<task-id>/context.md`:
...
5. Enter ORCHESTRATOR Planning Phase (Phase 1) for the first task:
...
6. After the first task closes (Phase 4), proceed to the next task if one was specified
