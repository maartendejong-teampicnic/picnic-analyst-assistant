...
```
~/Documents/Claude/analysistant/tasks/<task-id>/
...
0. Read `~/Documents/Claude/analysistant/user-config.md` — get `username_prefix` for task ID generation
1. Read `~/Documents/Claude/analysistant/TASKS.md` — identify the target task(s)
2. Resolve task-id(s) (see Task ID generation above)
3. Check for `~/Documents/Claude/analysistant/tasks/<task-id>/context.md`:
...
4. **Knowledge loading:** Read `~/Documents/Claude/analysistant/knowledge/INDEX.yaml`.
...
7. Create task folder and context file:
   ```
   mkdir ~/Documents/Claude/analysistant/tasks/<task-id>/
   ```
   Copy blank template from `~/Documents/Claude/analysistant/CONTEXT.md` →
   `~/Documents/Claude/analysistant/tasks/<task-id>/context.md`
   Fill in all fields (task title, id, status, brief, relevant files, subtask tracker)
8. Present to user:
   ```
   Plan for <task title>:
   Task ID: <task-id>
   Folder: analysistant/tasks/<task-id>/
...
c. Spawn via Agent tool with prompt:
   ...
   ~/Documents/Claude/analysistant/tasks/<task-id>/context.md. Execute your assignment exactly.
   Write all output to ~/.claude/data/agents/<task-id>/<role>/output.md
   (create the directory if it doesn't exist).
   Read ~/Documents/Claude/analysistant/agents/<ROLE>.md for your full onboarding.
   ```
...
| Domain | File | Type |
|--------|------|------|
| Picnic business vocabulary | `~/Documents/Claude/analysistant/context/picnic-business.md` | Shared |
| Communication style | `~/Documents/Claude/analysistant/context/communication-style.md` | Personal (gitignored) |
| Active project context | `~/Documents/Claude/analysistant/context/<project>.md` | Personal (gitignored) |
| Setup notes / MCP status | `~/Documents/Claude/analysistant/context/setup-notes.md` | Personal (gitignored) |
